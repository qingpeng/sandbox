...
#########################################################################################
if (@ARGV<3){
	print "Usage: program scaffold sfd_endlove od\n";
	exit;
}
...
while (<G>) {
	...
	if ($_ ne "") {
		...
		for ($i=0;$i<@sca-1 ;$i++) {
			$prm_name="BSBP_"."SUPER$sca_name"."_$end[$i]";
...
#			$seq2=substr($hash_ctg{$sca[$i+1]},length($hash_ctg{$sca[$i+1]})-$len2,length($hash_ctg{$sca[$i+1]})-1);
			...
			# ### modified by zhangqp #################¾àÀëgap150bp
			$target_start = $left_size-150;
			###########################################
