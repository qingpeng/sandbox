...
if(@ARGV<2){
	printf "Usage:program <position_file> <gene_desert.fa（UNMASKED!!)>  \nExample: perl ../bin/primers_v1.62.pl 7_0.in ../Gene_Desert/7_0.fa \n";
	exit;
}
...
my $right_seq;
my $desert_fasta;
($config_file,$desert_fasta) = (@ARGV);

my @s_tmp_a = split /\//,$config_file;
my @s_tmp_b = split /\./,$s_tmp_a[-1];
$dir = $s_tmp_b[0];
`mkdir $dir`;
$output = $dir."/$dir.primer.xls";
$for_blast = $dir."/$dir.primer.fasta";
$exon_file = $dir."/$dir.exon.fasta";
