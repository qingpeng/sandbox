...
if($ARGV[0] eq "1")
{
	...
	($type,$F_blat,$out_file) = @ARGV;

# 	open(F,$F_desert_position);
# 	while(<F>)
# 	{
# 		chomp;
# 		@temp = split(/\s+/,$_);
  ...
  $end{"0"} = 10000000000;
	...
	foreach $name (keys %start)
	{
		...
		while(<F>)
		{
			chomp;
			@temp = split(/\s+/,$_);
			if(@temp == 22) { shift(@temp); }
			if(($temp[15] > $start{$name} && $temp[15] < $end{$name}) || ($temp[16] > $start{$name} && $temp[16] < $end{$name}))
			{
				$strand = $temp[8];
				$query = $temp[9];
				chop($temp[18]);
				chop($temp[20]);
				@length = split(/\,/,$temp[18]);
				@start = split(/\,/,$temp[20]);
				@posi = ();
				for($i=0;$i<@start;$i++)
				{
				...
				if($strand eq "+")
				{
					...
					for($i=0;$i<@posi;$i+=2)
					{
						if(($posi[$i] > $start{$name} && $posi[$i+1] < $end{$name}))
						{
							$a=$posi[$i]-$start{$name}+1;
							$b=$posi[$i+1]-$start{$name}+1;
							$out = "$a:$b:+::$query\n";
							push(@out,$out);
						}
					}
					if(@out>0) { $out[$#out] =~ s/\+/\+1/; }
				}
				else
				{
					$out = "";
					...
					for($i=0;$i<@posi;$i+=2)
					{
						...
						{
							$a=$posi[$i]-$start{$name};
							$b=$posi[$i+1]-$start{$name};
							$out = "$a:$b:-::$query\n";
							push(@out,$out);
						}
					} #print "@out\n";
					if(@out>0) { $out[0] =~ s/\-/\-1/; } #print "@out\n";
				}
				print O @out;
			}
		}
