...
# v1.1 add excluded region and target!
...
for (my $n = 1;$n<$seq_num+1;$n++) {
	...
	if ($length[$n]>=100) {# if length >100 ,exclude 10 只有长度>100时，才屏蔽前10
		my $start_r = $length[$n]-10;
		$config{EXCLUDED_REGION}="0,10 $start_r,10";
	}
...
# 几个限制条件 ： joint sequence 位于中间N两边300bp区域，序列长于100bp,屏蔽 两边缘的10 bp
for (my $n = 1;$n<$seq_num;$n++) {
...
#	}
	...
	if ($length[$n]>100 && $length[$n]<310) {
		$excluded = " 0,10";
		$config{EXCLUDED_REGION}=$config{EXCLUDED_REGION}.$excluded;
	}
	if ($length[$n+1]>100 && $length[$n+1]<310) {
		$start4 = $length[$n]+100+$length[$n+1]-10;
		$excluded = " $start4,10";
		$config{EXCLUDED_REGION}=$config{EXCLUDED_REGION}.$excluded;
	}
...
open IN,"/disk10/prj0326/zhangqp/prj_2003-11_Big_Gene/Primer/Gene_Desert/$desert_name" || die"$!";
