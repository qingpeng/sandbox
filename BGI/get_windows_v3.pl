...
# 2005-4-6 19:33
# # 至少两个物种有的repeat
# human genome上windows<1000 与后面的合并
# 2005-4-7 11:44
# 
if (@ARGV<4) {
...
#bsab_human_chr3	12174459	12174769	C	L2	LINE/L2
		...
		close REPEAT;
		@repeat_name = keys %deer;
		foreach my $repeat_name (sort @repeat_name) { 
			print  "$repeat_name\n";
			if (exists $human{$repeat_name}|| exists $dog{$repeat_name} || exists $mouse{$repeat_name} ) { # 四个物种中至少出现三次 
				if ($print_m ==0) {
